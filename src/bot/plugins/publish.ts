...
>(options?: PublishOptions) {
    return CommandInitPlugin<T>(async ({ module }) => {
        ...
        options = { ...defaultOptions, ...options } as PublishOptions &
            ValidPublishOptions;
        const { defaultMemberPermissions, dmPermission, guildIds } =
        ...
        const log =
            (...message: Array<unknown>) =>
                () =>
                    console.log(...message);
        const logged = (...message: Array<unknown>) => log(message);
        /**
        ...
        try {
            ...
            if (!guildIds.length) {
                const cmd = (await client.application!.commands.fetch()).find(
                    (c: { name: string; type: ApplicationCommandType }) =>
                        c.name === module.name && c.type === curAppType,
                );
                if (cmd) {
...
export type NonEmptyArray<T extends `${number}` = `${number}`> = [
    T,
    ...Array<T>,
];
